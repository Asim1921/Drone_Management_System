...
const LightPillar = ({
  ...
  useEffect(() => {
    const checkWebGL = () => {
      try {
        ...
        // Test if we can actually create a context
  ...
  useEffect(() => {
    if (!containerRef.current || webGLSupported === false || webGLSupported === null) return;
    ...
    try {
      // Suppress WebGL errors during renderer creation
      const originalError = console.error;
      const errorMessages: string[] = [];
      
      console.error = (...args: any[]) => {
        const errorString = args.join(' ');
        // Only suppress WebGL context creation errors
        if (errorString.includes('WebGL context') || 
            errorString.includes('WebGLRenderer') ||
            errorString.includes('Could not create a WebGL context')) {
      ...
      // Restore console.error after successful creation
      console.error = originalError;
      
      // If we captured WebGL errors but renderer was created, it might be a warning
      if (errorMessages.length > 0) {
        // Renderer was created despite warnings, so we continue
      }
    } catch (error) {
      setWebGLSupported(false);
      return;
  ...
  // Show nothing if WebGL is not supported (graceful degradation)
